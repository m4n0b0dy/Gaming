...
for dex, dic_seg in enumerate(batches):
	...
	if not (set(dic_seg.keys()) < set(completed_games) or set(dic_seg.keys()) == set(completed_games)):
		print("Batch #%s of %s, Batch Size %s of %s total games - %s seconds" % (dex+pc_strt,len_bathces,len(dic_seg),len_games,round(time.time() - start_time,4)))
		#run through a batches game image links or game urls, download and save
		run_and_save(dic_seg, attr_to_pull, func_dic[attr_to_pull], str(dex+pc_strt))
		save_j(list(dic_seg.keys())+completed_games,attr_to_pull+'/completed_games')
	else:
		print("Batch #%s is already loaded in %s/" % (str(dex+pc_strt),attr_to_pull))